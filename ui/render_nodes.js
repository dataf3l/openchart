...
function renderNodes(){
	...
	for(var n=0;n<globalNetwork.nodes.length;n++){
		...
		dx += "<div onmouseup=\"updateNodePosition("+n+",this);\" id=\"node"+n+"\" class=node style='position:absolute;left:"+node.x+"px;top:"+node.y+"px;width:"+Math.round(node.width)+"px;height:"+Math.round(node.height)+"px' onclick='nodeClicked("+n+")'>"+node.name+"("+node.type+")"+"</div>";
	}
